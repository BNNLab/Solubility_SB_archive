...
##get training sets
water_loc1_train="model_data/water/water1_gen1_PM6_train.csv"
ethanol_loc_train="model_data/ethanol/ethanol_gen1_PM6_train.csv"
benzene_loc_train="model_data/benzene/benzene_gen1_PM6_train.csv"
acetone_loc_train="model_data/acetone/acetone_gen1_PM6_train.csv"
...
def get_preds_data(test,train,solvent):    
    preds=[]
    preds.append(get_pred("model_data/" + solvent + "/ET_MP_PM6.dump",train,test))
    preds.append(get_pred("model_data/" + solvent + "/SVM_MP_PM6.dump",train,test))
    preds.append(get_pred("model_data/" + solvent + "/MLP_MP_PM6.dump",train,test))
    gpr_preds,lower,upper=get_pred_GPR("model_data/" + solvent + "/GPR_MP_PM6.dump",train,test)
...
def get_preds_data_noMP(test,train,solvent):    
    preds=[]
    preds.append(get_pred_noMP("model_data/" + solvent + "/ET_noMP_PM6.dump",train,test))
    preds.append(get_pred_noMP("model_data/" + solvent + "/SVM_noMP_PM6.dump",train,test))
    preds.append(get_pred_noMP("model_data/" + solvent + "/MLP_noMP_PM6.dump",train,test))
    gpr_preds,lower,upper=get_pred_GPR_noMP("model_data/" + solvent + "/GPR_noMP_PM6.dump",train,test)
    preds.append(gpr_preds)
    mean=np.mean(preds)
    median=np.median(preds)
    preds.append(upper)
...
def get_final_pred_PM6(path,gas_log,sol_log,MP):
